...
# coding: utf-8
'''
reference 1 web server by wsgi protocol  http://python.jobbole.com/81524/
                                         http://python.jobbole.com/81523/
                                         http://python.jobbole.com/81820/
          23  wsgi protocol     https://segmentfault.com/a/1190000002717571
'''
import socket
import sys
import datetime
...
class WSGIServer(object):
    ...
    def handle_request(self):
        ...
        try:
            ...
            for value in self.application(env,self.start_response):
                self.data.append(value)
            data=''.join(self.data)
            self.finish_response(data)
            print '[{0}] "{1}" {2}'.format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),self.request_lines[0], self.status)
        except Exception, e:
            pass
...
if __name__ == '__main__':
    ...
    application = getattr(module, application)# equal module.application
